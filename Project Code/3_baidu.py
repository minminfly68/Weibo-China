...
if __name__ == '__main__':
    APP_ID = '032e9b861ba64acfa840ca567d4c942b'
    API_KEY = 'SxdveFzFNEAEyppta563vKGp'
    SECRET_KEY = 'WAdcZ6OWhfBGiCfDhTO3yuCVjDX5I2gf'
    ...
    for row in df.itertuples():
        result=client.sentimentClassify(row[13])
        if 'items' in result:
            temp_sentiment = result['items'][0]['sentiment']
            temp_confidence = result['items'][0]['confidence']
        else:
            temp_sentiment = 9
            temp_confidence = 9
        print(temp_confidence, temp_sentiment)
        ls_1.append(temp_sentiment)
        ls_2.append(temp_confidence)

    df['sentiment'] = ls_1
    df['confidence'] = ls_2
    df.to_csv("output2.csv", index=False, encoding='utf_8_sig')
